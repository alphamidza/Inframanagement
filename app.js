...
// Add this code inside your DOMContentLoaded event listener, after your other main functions

function handleAddFeature() {
    const panel = document.getElementById('analysis-results');
    panel.innerHTML = `
        <h4>Add New Feature</h4>
        <p>Select feature type, then click on the map to add it.</p>
        <form id="add-feature-form">
            <div class="form-group">
                <label for="feature-type">Feature Type:</label>
                <select id="feature-type" required>
                    <option value="">Select</option>
                    <option value="node">Node</option>
                    <option value="pipe">Pipe</option>
                </select>
            </div>
            <div id="add-feature-fields"></div>
            <button type="submit" class="btn-primary">Add Feature</button>
        </form>
        <div id="add-feature-status" style="margin-top:10px;"></div>
    `;

    let addFeatureMapClick = null;
    let selectedPoints = [];
    let tempLayer = null;

    // Listen for feature type selection
    document.getElementById('feature-type').addEventListener('change', function() {
        ...
        const fieldsDiv = document.getElementById('add-feature-fields');
        if (type === "node") {
            fieldsDiv.innerHTML = `
                <div class="form-group">
                ...
                <div class="form-group">
                    <label for="node-elevation">Elevation (m):</label>
                    <input type="number" id="node-elevation">
                </div>
                <p>Click on the map to select the node location.</p>
            `;
            if (addFeatureMapClick) map.off('click', addFeatureMapClick);
            addFeatureMapClick = function(e) {
                selectedPoints = [e.latlng];
                if (tempLayer) map.removeLayer(tempLayer);
                tempLayer = L.circleMarker(e.latlng, { radius: 8, color: '#2ecc71', fillColor: '#2ecc71', fillOpacity: 0.8 }).addTo(map);
                document.getElementById('add-feature-status').textContent = `Selected location: Lat ${e.latlng.lat.toFixed(6)}, Lng ${e.latlng.lng.toFixed(6)}`;
            };
            map.on('click', addFeatureMapClick);
        } else if (type === "pipe") {
            fieldsDiv.innerHTML = `
                <div class="form-group">
                    <label for="pipe-id">Pipe ID:</label>
                    <input type="text" id="pipe-id" required>
                </div>
                <div class="form-group">
                    <label for="pipe-diameter">Diameter (mm):</label>
                    <input type="number" id="pipe-diameter">
                </div>
                <div class="form-group">
                    <label for="pipe-roughness">Roughness:</label>
                    <input type="number" id="pipe-roughness">
                </div>
                <p>Click <b>two points</b> on the map to define the pipe (start and end).</p>
            `;
            if (addFeatureMapClick) map.off('click', addFeatureMapClick);
            addFeatureMapClick = function(e) {
                if (selectedPoints.length < 2) {
                    ...
                    if (selectedPoints.length === 1) {
                        tempLayer = L.circleMarker(e.latlng, { radius: 8, color: '#e67e22', fillColor: '#e67e22', fillOpacity: 0.8 }).addTo(map);
                        document.getElementById('add-feature-status').textContent = `Start selected: Lat ${e.latlng.lat.toFixed(6)}, Lng ${e.latlng.lng.toFixed(6)}. Now click end point.`;
                    } else if (selectedPoints.length === 2) {
                        tempLayer = L.polyline([selectedPoints[0], selectedPoints[1]], { color: '#e67e22', weight: 6 }).addTo(map);
                        document.getElementById('add-feature-status').textContent = `Start: Lat ${selectedPoints[0].lat.toFixed(6)}, Lng ${selectedPoints[0].lng.toFixed(6)} | End: Lat ${selectedPoints[1].lat.toFixed(6)}, Lng ${selectedPoints[1].lng.toFixed(6)}`;
                    }
                }
            };
            map.on('click', addFeatureMapClick);
    ...
    // Handle form submission
    document.getElementById('add-feature-form').addEventListener('submit', async function(event) {
        event.preventDefault();
        const type = document.getElementById('feature-type').value;
        const statusDiv = document.getElementById('add-feature-status');
        if (type === "node") {
            if (selectedPoints.length !== 1) {
                statusDiv.textContent = "Please select the node location on the map.";
            ...
            const node = {
                ...
                geometry: {
                    type: "Point",
                    coordinates: [selectedPoints[0].lng, selectedPoints[0].lat]
                },
                properties: {
                    node_id: document.getElementById('node-id').value,
                    node_type: document.getElementById('node-type').value,
                    elevation: document.getElementById('node-elevation').value
                }
            };
            // Optionally: send to backend or JSONbin (here we'll just show success)
            statusDiv.textContent = `Node added: ${JSON.stringify(node)}`;
            statusDiv.style.color = "green";
            // You can push to json_harare_nodes2_2.features if you want
            // json_harare_nodes2_2.features.push(node);
            if (tempLayer) tempLayer.setStyle({ color: '#2ecc71', fillColor: '#fff' });
            map.off('click', addFeatureMapClick);
        } else if (type === "pipe") {
            if (selectedPoints.length !== 2) {
                statusDiv.textContent = "Please select two points on the map for the pipe.";
                statusDiv.style.color = "red";
                return;
            }
            // Collect pipe data
            const pipe = {
                ...
                geometry: {
                    type: "LineString",
                    coordinates: [
                        [selectedPoints[0].lng, selectedPoints[0].lat],
                        [selectedPoints[1].lng, selectedPoints[1].lat]
                    ]
                },
                properties: {
                    pipe_id: document.getElementById('pipe-id').value,
                    diameter: document.getElementById('pipe-diameter').value,
                    roughness: document.getElementById('pipe-roughness').value,
                    start_node_coords: [selectedPoints[0].lng, selectedPoints[0].lat],
                    end_node_coords: [selectedPoints[1].lng, selectedPoints[1].lat]
                }
            };
            statusDiv.textContent = `Pipe added: ${JSON.stringify(pipe)}`;
            statusDiv.style.color = "green";
            // json_harare_pipes_1.features.push(pipe);
            if (tempLayer) tempLayer.setStyle({ color: '#e67e22' });
            map.off('click', addFeatureMapClick);
        }
    });

    // Clean up on panel close or when leaving add feature mode
    // (optionally add such a handler if you have a panel manager)
}

// Attach to the Add Feature button (ensure your HTML button has id="add-feature")
document.getElementById('add-feature').addEventListener('click', handleAddFeature);
